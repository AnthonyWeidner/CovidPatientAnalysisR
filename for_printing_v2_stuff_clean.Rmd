...
new_data <- read_excel("C:\\Users\\antho\\Desktop\\Gong Lab\\MassSpec_name_simplified_no_NA.xlsx")
...
numeric_data <- counts[,-1]  # Exclude the first column
...
```{r}
write.csv(v2_top.table, file = "Covid_2_wk_vs_Control.csv")
```
...
```{r}
write.csv(v2_top.table2, file = "Covid_6_wk_vs_Control.csv")
```
...
png(file = "VolcanoPlot_Cov2wk_vs_Control.png", res = 300, width = 8, height = 6, units = "in")
...
png(file = "ViolinPlot_Cov2wk_vs_Control.png", res = 300, width = 6, height = 6, units = "in")
...
for (curr in v2_top.table$Gene[1:29]) {
...
gene_df$Group <- ifelse(grepl("ctrl", gene_df$Sample), "Control_No_Covid",
...
# Filter out the "Covid_6_weeks" group
gene_df <- gene_df %>%
  filter(Group %in% c("Covid_2_weeks", "Control_No_Covid"))
...
png(file = gsub(" ", "", paste(gene_of_interest, ".png")), res = 300, width = 6, height = 6, units = "in")


# ... [Rest of your data preparation code remains unchanged]

# Create the box plot for each group
plt <- ggplot(gene_df, aes(x = Group, y = Expression, fill = Group)) +
  geom_boxplot() +
  ...
  scale_fill_manual(values = c("Covid_2_weeks" = "red", "Control_No_Covid" = "cyan"))
